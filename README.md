...
|-------------------|---------------------|-----------------|----------------------|------------------|----------------------------------------------------|
| *None*            | 長條圖部分標籤消失           | 邊緣 (Minor)      | 即將發布 (Fixed)         | 無                | 在顯示長條圖時，「交通出行 / 工資薪水」標籤會因為不明原因無法正常顯示，但不影響實際結果      |
...
|--------------|-----------------------------------------------------|-----------------|------------|---------------|
| ***1.1.16*** | 修復了許多問題與潛在漏洞，顯著提升程式運行時的穩健性                          | 功能測試 (Testing)  | 中 (Medium) | ***2025-06*** |
| 1.1.17       | 在 `Func.py` 的 `analyze()` 中新增顯示特定期間的資產變化（總收入/總支出）功能 | 功能規劃 (Planning) | 高 (High)   | 2025-06       |
