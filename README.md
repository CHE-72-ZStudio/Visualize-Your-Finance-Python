...
|-------------|--------------------------------------------------------------------|-------------------|------------|---------------|
| ***1.3.7*** | 在 `Func.py` 的 `analyze()` 中新增「總體次數折線走勢圖」、「各類次數折線走勢圖」、「總體金額／次數表格」功能 | 正在開發 (Developing) | 中 (Medium) | ***2025-08*** |
| ***1.3.7*** | 在資料夾中沒有 `Record.csv` 時，可以自動新增該檔案，避免因缺少檔案而無法運行                      | 準備發布 (Completed)  | 中 (Medium) | ***2025-08*** |
| ***1.3.7*** | 調整 `USER.md` 中的說明方式，使其更為完整易懂                                       | 正在設計 (Designing)  | 低 (Low)    | ***2025-08*** |
| ***1.3.7*** | 優化 `Func.py` 中「細項排名表」的顯示方式，以實際數量作為排名榜單的標題                          | 準備發布 (Completed)  | 中 (Medium) | ***2025-08*** |
| ***1.3.7*** | 優化折線圖與長條圖的背景顯示顏色為白煙色，增加圖表可視度                                       | 準備發布 (Completed)  | 低 (Low)    | ***2025-08*** |
| 1.4.0       | 增加可以寫入 `Diagnose.Log` 日誌檔案的功能                                      | 功能規劃 (Planning)   | 中 (Medium) | 2025-??       |
| 1.4.1       | 增加更多例外情形處理                                                         | 正在討論 (Discussing) | 低 (Low)    | 2025-??       |
| 1.4.2       | 可執行檔集成華為 HarmonyOS Sans 字體                                         | 正在討論 (Discussing) | 低 (Low)    | 2026-??       |
| 1.4.5       | 提升跨系統顯示相容性，並開始發行 macOS/Linux 可執行檔                                  | 功能規劃 (Planning)   | 低 (Low)    | 2026-??       |
| 1.5.0       | 調整優化程式邏輯與簡化選單操作介面，為即將新增的 GUI 功能做準備                                 | 功能規劃 (Planning)   | 低 (Low)    | 2026-??       |
