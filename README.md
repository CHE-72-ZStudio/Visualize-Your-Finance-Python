...
|-------------------|---------------------|-----------------|----------------------|------------------|----------------------------------------------------|
| *None*            | 「新增支出／收入紀錄」功能無法使用   | 部分 (Major)      | 即將發布 (Fixed)         | 無                | 在「新增支出／收入紀錄」時，會出現異常的「您的輸入內容出現其他錯誤」，導致無法正常使用功能      |
...
|--------------|--------------------------------------------------------------------|-------------------|------------|---------------|
| ***1.3.12*** | 在 `Func.py` 的 `analyze()` 中新增「總體次數折線走勢圖」、「各類次數折線走勢圖」、「總體金額／次數表格」功能 | 準備發布 (Completed)  | 中 (Medium) | ***2025-08*** |
| ***1.3.12*** | 在資料夾中沒有 `Record.csv` 時，可以自動新增該檔案，避免因缺少檔案而無法運行                      | 準備發布 (Completed)  | 中 (Medium) | ***2025-08*** |
| ***1.3.12*** | 調整 `USER.md` 中的說明方式，使其更為完整易懂                                       | 正在設計 (Designing)  | 低 (Low)    | ***2025-08*** |
| ***1.3.12*** | 優化 `Func.py` 中「細項排名表」的顯示方式，以實際數量作為排名榜單的標題                          | 準備發布 (Completed)  | 中 (Medium) | ***2025-08*** |
| ***1.3.12*** | 優化折線圖與長條圖的背景顯示顏色為白煙色，增加圖表可視度                                       | 準備發布 (Completed)  | 低 (Low)    | ***2025-08*** |
| ***1.3.12*** | 在使用「顯示目前資產」功能時，同時顯示 支出／收入 的 總金額／總筆數                                | 準備發布 (Completed)  | 低 (Low)    | ***2025-08*** |
| ***1.3.12*** | 優化圓餅圖的顯示效果，有效避免 類別標籤／比例數值 擠在一起，導致文字難以辨識的問題                         | 正在開發 (Developing) | 低 (Low)    | ***2025-08*** |
| ***1.3.12*** | 修復了在「新增支出／收入紀錄」功能中出現異常，無法正常使用的問題                                   | 準備發布 (Completed)  | 高 (High)   | ***2025-08*** |
| ***1.3.12*** | 修復了在程式運行中，部分文字輸出內容出現邏輯不一致、存在前後矛盾的問題                                | 準備發布 (Completed)  | 低 (Low)    | ***2025-08*** |
| ***1.3.12*** | 優化程式中的 金額／排名 數字顯示效果，現在會自動顯示 `,` 分隔符，方便檢視與數值判讀                      | 準備發布 (Completed)  | 低 (Low)    | ***2025-08*** |
