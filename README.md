...
|--------------|------------------------------------------------------|------------------|------------|---------------|
| ***1.0.11*** | 修復開啟 UTF-8 編碼的 `Record.csv` 會出現無法解碼的錯誤               | 準備發布 (Completed) | 高 (High)   | ***2025-06*** |
| ***1.0.11*** | 在 `Func.py` 中的 `cat_question()` 「類別選擇平臺」中新增邊界檢查      | 準備發布 (Completed) | 高 (High)   | ***2025-06*** |
| ***1.0.11*** | 在 `Func.py` 中的 `analyze()`「時間選擇平臺」中輸入年、月、日時新增邊界檢查    | 準備發布 (Completed) | 高 (High)   | ***2025-06*** |
| ***1.0.11*** | 在 `Studio.py` 中新增檢查，避免使用者誤啟動該模組                      | 準備發布 (Completed) | 中 (Medium) | ***2025-06*** |
| ***1.0.11*** | 在 `Main.py` 中新增一些驚喜的小彩蛋，供使用者發掘，提升用戶體驗                | 準備發布 (Completed) | 低 (Low)    | ***2025-06*** |
| ***1.0.11*** | 調整 CLI 輸出時的顏色變化                                      | 準備發布 (Completed) | 高 (High)   | ***2025-06*** |
| ***1.0.11*** | `.gitignore` 中新增 Windows 與 macOS 的模板，避免提交操作系統產生的快取檔案 | 準備發布 (Completed) | 中 (Medium) | ***2025-06*** |
| ***1.0.11*** | `.gitignore` 中新增 `Record.csv`，避免提交開發人員私人的帳目資料        | 準備發布 (Completed) | 中 (Medium) | ***2025-06*** |
| ***1.0.11*** | 修正 `Main.py`、`MANUAL.md` 中部分功能的描述說明                  | 準備發布 (Completed) | 中 (Medium) | ***2025-06*** |
| ***1.0.11*** | 新增 `Example.csv` 檔案，供使用者參考                           | 準備發布 (Completed) | 中 (Medium) | ***2025-06*** |
| ***1.0.11*** | 因已完成作業繳交，移除 `MakeRecord.py` 檔案，使其符合一般使用者的使用方式        | 準備發布 (Completed) | 低 (Low)    | ***2025-06*** |
