...
|--------------|----------------------------------------------------|------------------|------------|---------------|
| ***1.0.15*** | 在 `Func.py` 中的 `analyze()`「時間選擇平臺」中新增更為完善的邊界檢查     | 準備發布 (Completed) | 中 (Medium) | ***2025-06*** |
| ***1.0.15*** | 在 `Main.py` 中計算資產時，現在會使用分隔位顯示金額，提升可閱讀性與清晰度         | 準備發布 (Completed) | 低 (Low)    | ***2025-06*** |
| ***1.0.15*** | 在 `README.md` 中新增對於「Ask DeepWiki」徽章的描述與免責聲明        | 準備發布 (Completed) | 低 (Low)    | ***2025-06*** |
| ***1.0.15*** | 修正 `MANUAL.md` 中關於「分析選擇平臺」的功能說明，使其與 `Func.py` 保持一致 | 準備發布 (Completed) | 低 (Low)    | ***2025-06*** |
